...
const PANEL_USERNAME = '2profess321';
...
async function launchBrowser() {
	if (!browser) {
		...
		await page.goto('https://fairbet9.com/admin');
...
async function createNewUser(item) {
	...
	if (aff_user_id) {
		try {
			...
			await newPage.goto('https://fairbet9.com/admin/users', { waitUntil: 'networkidle2' });
		...
		if (type && ["create", "deposit", "withdraw"].includes(type)) {
			try {
				...
				if (!testflight) {
					...
					if (currentUrl === "https://fairbet9.com/admin") {
						...
						var newUrl = page.url();
						if (newUrl !== "https://fairbet9.com/admin") {
